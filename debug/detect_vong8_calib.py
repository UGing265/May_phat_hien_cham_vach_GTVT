...
def process_frame(frame, frame_idx, calibrated_ellipse, ellipse_buffer):
    ...
    - Nếu đã calibrate: chỉ vẽ ellipse cố định, không tìm contour nữa.
    """
...
def main():
    ...
    if USE_IMAGE:
        ...
        # Với ảnh tĩnh: detect 1 lần, không cần calib
        ...
        cv2.namedWindow("Vong 8 - Video (Calib + Run)", cv2.WINDOW_NORMAL)
        ...
        cv2.createTrackbar("Seek", "Vong 8 - Video (Calib + Run)", 0, total_frames - 1, on_trackbar)
        ...
        while True:
            ...
            cv2.setTrackbarPos("Seek", "Vong 8 - Video (Calib + Run)", current_pos)

            frame_idx += 1

            cv2.imshow("Vong 8 - Video (Calib + Run)", show)
        ...
        while True:
            ...
            cv2.imshow("Vong 8 - Webcam (Calib + Run)", show)
            key = cv2.waitKey(1) & 0xFF
            if key in (27, ord("q")):
                break
